...
# 3. else 문
# if문의 조건이 충족되지 않을 대(즉, if문의 조건이 False일 때) 실행되는 코드 블록을 지정
'''
if 조건:
    조건이 True일 때 실행할 코드
else:
...
# x and y  x와 y가 모두 참이여야 True
# x or y   x나 y가 둘중 하나만 참이라도 True 둘다 False인경우에만 False
# not x    x가 False라면 True  x가 True라면 False가 출력
...
# 이전 if문 또는 elif문 조건이 False일 때 확인되는 추가 조건을 지정
...
# 어던 수가 10보다 큰지, 10인지, 또는 10보다 작은지를 확인하는 코드
num = 10
